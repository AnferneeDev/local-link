...
import { File, Send, Type, Loader2, FileText, UploadCloud } from "lucide-react";
import { useAppContext } from "../context/AppContext";
import { tButton } from "../lib/translations"; // Import tButton

// --- 1. UPDATED: Button Text Components ---
// (We now import tButton from translations.ts)
...
const FileUpload = () => {
  const { lang, t, handleFileChange, handleChooseFileClick, fileInputRef, handleUploadClick, selectedFile, statusType } = useAppContext();
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="text-center">
          {selectedFile ? (
            <div className="flex flex-col items-center text-slate-700 dark:text-slate-200">
              ...
              <span className="mt-1 text-sm font-medium truncate max-w-xs">{selectedFile.name}</span>
            </div>
          ) : (
      ...
      <Button className="w-full py-5 rounded-2xl font-semibold text-lg bg-linear-to-r from-indigo-600 to-pink-500 hover:from-indigo-500 hover:to-pink-40AF" onClick={handleUploadClick} disabled={!selectedFile || statusType === "uploading-file"}>
        <FileButtonText />
      </Button>
    </div>
  );
...
const TextUpload = () => {
  ...
  return (
    <div className="space-y-3">
      <Textarea id="text-input" placeholder="Type your message or paste a link..." value={text} onChange={(e) => setText(e.target.value)} className="resize-none h-24 overflow-y-auto" rows={3} />
      <Button
